                ...
                if file.endswith('.lnk'):
                    ...
                    try:
                        ...
                        if target_path:
                            ...
                            if not app_name.lower().startswith("uninstall"):
                                if (app_name not in apps
                                    and app_name not in settings.get("hidden_apps", [])):
                                        apps[app_name] = {'path': lnk_path, 'args': None}
                    except Exception:
                        continue
...
max_width_window = int(screen_width * 0.9)
max_height_window = int(screen_height * 0.9)

window_width = min(required_width, max_width_window)
window_height = min(required_height, max_height_window)

if required_height > max_height_window:
    root.geometry(f"{window_width}x{max_height_window}")
...
x = work_area.left
y = work_area.bottom - window_height - 60
